...
2. 读 `candidates/{id}/data/changelog.jsonl` —— 每次变异一条 `ChangeLogEntry`,带 `subject`/`body`/`diff`/`files_modified`,**这就是可调产物演化史的可读叙述**。
...
| 变异 prompt | `antomnievo/proposer/template/mutation_prompt.py` | actions 该怎么落到可调产物上 |
| artifact 约定 | `antomnievo/model/tunable_artifact_defs/<your>_tunable_artifact_def.py` | 可调产物允许哪些文件、每个文件什么含义 |

常见失效模式:analysis 诊断了问题但没覆盖它的 `action`;变异在部分题上修好但别的题改坏了(对比逐题 delta);可调产物写对了但系统没照着做(看 child 的 trajectory)。
