...
|---|---|---|---|
| target | `System` | Run the system under optimization (agent / workflow / single-file algorithm…) with the candidate tunable artifacts; return `SystemResult` (trajectory + output + usage) | `ReactAgentSystem` (in-process) / `Text2SQLSystem` / `AppWorldSystem` / `TerminalBenchSystem` (subprocess) |
...
| optimizer | `Proposer` | The coding agent that reads failures and rewrites the tunable artifacts | `ClaudeCodeProposer` (drives `claude`), `PiCodingAgentProposer` (drives `pi`) |
...
| persistence | `CandidateStore` | Persist all state (tunable-artifact tree, runs, analysis, changelog, statistics) | `LocalCandidateStore` (filesystem) |
...
- **Cloud storage**: `CandidateStore` is an abstract ABC with a default `LocalCandidateStore` that keeps the evolution memory (tunable-artifact tree, meta, changelog, statistics) on the local `workspace/`. One optimization run = one `Optimizer` process + one local store. To plug into cloud infra, **implement a "local + transparent-sync" `CandidateStore` subclass**: fully transparent to the optimizer, it still runs on the local `workspace/`, and on every write it also syncs artifacts to object storage / a DB (pull the initial tunable artifacts before the run; push artifacts for downstream consumption afterwards). The optimizer main loop and its concurrency (`num_proposals` slots inside one process) are unchanged. **Note**: this is *not* "multiple workers sharing one store for cross-host concurrent evolution" — that's a different model the framework does not support out of the box.
...
flowchart LR
    ...
    subgraph Cloud ["Cloud storage (a sync subclass you implement)"]
        direction TB
        OBJ[("object storage / DB<br/>tunable-artifact tree + meta/changelog/best")]
    end
    Local -.transparent sync on write.-> OBJ
    OBJ -.pull initial tunable artifacts before run<br/>expose artifacts after.-> Downstream([downstream])
```

## 3. The 7 components: which to write yourself

The seven pluggable components `Optimizer` assembles — **target side** (`System` / `Evaluator` / `DataInst`), **optimizer side** (`Optimizer` loop / `Proposer` / `EvolutionAlgorithm`), **persistence** (`CandidateStore`) — plus the **tunable-artifact schema** (`TunableArtifactSchema`) you lay out separately:

| # | Component | Default impl | Write your own? |
|---|---|---|---|
| 1 | `System` | 4 scenario references | **Yes** — run your system with a candidate's `artifact_dir`; return `SystemResult`. |
...
> **`TunableArtifactSchema`** (`antomnievo/model/tunable_artifact_defs/<your>_tunable_artifact_def.py`) is **not** one of the seven — it's the **tunable-artifact declaration**: with `FolderSchema` / `FileSchema` you declare the artifact directory tree and what each file is for, which is both the proposer's only source for "which files it may touch" *and* the mapping of your system's tunable artifacts onto that directory. You **must** write it for a new task (same standing as `System` / `Evaluator` / `DataInst`), but it's a declarative schema you lay out, not a pluggable component.
>
> The **initial tunable-artifact directory** (`tasks/<your>-base/`, the root candidate's seed) and the **entry-point script** (wires components + data + hyperparameters and calls `await optimizer.optimize()`) are also inputs you provide, not components.
...
- **`System`** (`antomnievo/system/<your>/`): subclass `System` and either implement `_run` (in-process) **or** override `run_batch` (subprocess). The load-bearing input is `candidate_meta.artifact_dir` — your system must read its tunable artifacts from that directory.
...
- **`TunableArtifactSchema`**: see the note above (`antomnievo/model/tunable_artifact_defs/<your>_tunable_artifact_def.py`, `FolderSchema`/`FileSchema`).
