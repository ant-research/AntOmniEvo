...
| `{spec_dir}` | ✅ Read only | CURRENT spec — `{last_candidate_id}`'s spec (cumulative result of every chain mutation). |
| `{analysis_result_path}` | ✅ Write only | Your output file — write the RunAnalysis JSON here. |

- You may read: the run files listed below, the chain candidate run files listed below, the spec directory, and the analysis result file.
...
3. Map that changelog entry to the current spec — is the successful change still present? If it was removed by a later candidate, that explains regression.
...
- **+0.3, -0.3, +0.3, -0.3** → Two spec changes are in conflict. One helps this data_id, the other hurts it. The chain keeps swapping between them. You must propose a solution that handles BOTH needs simultaneously (scoped conditions, conditional logic, different mechanism).
...
### Method 4: Inconsistency Pattern Discovery (Same-Spec Variance)

When multiple run records exist for the SAME candidate on this data_id and show different outcomes (some score 1.0, some score 0.0, or scores vary significantly across runs), this is an inconsistency pattern. It is NOT a lost win and NOT oscillation — the spec itself produces both correct and incorrect outputs non-deterministically. This is one of the most valuable optimization opportunities, because the successful runs prove the spec CAN produce correct outputs; your job is to make that the RELIABLE output.
...
5. **Propose a hardening action**: Convert the winning path into a deterministic rule or guardrail in the spec. Common patterns:
   ...
   - If the spec already contains the correct guidance but the system still sometimes ignores it, add a stronger trigger (priority placement, mandatory step, explicit cross-reference).

**Do NOT dismiss inconsistent tasks as "model variance" and skip analysis.** The fact that the system sometimes gets it right proves the issue is in the spec's ability to RELIABLY trigger the correct path, not in the model's capability. Every inconsistent data_id is a concrete optimization target.
...
2. **Success Extraction**: If any candidate improved this data_id, write down: which candidate, what it did, whether that change is still in the current spec.
...
| D | The new content conflicts with or is overridden by other spec content — often an earlier chain entry (SUPPRESSION pattern: an earlier attempt improved data_id A while degrading data_id B, the latest attempt tried to fix B and re-broke A) | Resolve the conflict — remove or revise the conflicting piece. Cite WHICH earlier candidate id's entry it conflicts with. |
...
| I | The spec already contains content that addresses this issue, but the system did not follow it — the content is present, correct, and accessible, yet the trajectory shows the system bypassing it. Detectable when: the action's `spec_issue` references content that already exists in the spec. | The component's mechanism is too weak to guarantee the desired behavior. Move the fix to a component with a stronger mechanism per the Spec Schema — do NOT add more content at the same component. Cite the existing content in `spec_issue` to prove it already exists. |
| F | The chain has already tried this DIRECTION multiple times with no success (visible as Δ_{{root}}→{{cid}} ≤ 0 for every chain candidate in the score row) | Propose a qualitatively different approach — different spec component, different mechanism. Consult the Spec Schema for available components with stronger guarantees. |
| G | The runs show inconsistent results for this data_id under the same spec (some runs succeed, others fail, or scores vary significantly across runs) | Do NOT dismiss as "model variance" and skip. Apply Method 4 (Inconsistency Pattern Discovery): compare a successful and a failed run, identify the divergence point, and propose actions that harden the winning path into a deterministic rule or guardrail. Only leave `trajectory_analysis` and `actions` empty if ALL runs agree on the outcome (all succeed or all fail). |
...
1. **Prefer REMOVE / REPLACE over ADD.** If diagnosis B, C, or D applies, the corrective action targets existing content. Layering more on top of failed content usually makes the spec noisier and the system more confused.
2. **Cite the responsible chain candidate in `spec_issue`.** Beyond the per-action diagnosis, name the candidate id whose changelog entry introduced (or removed) the defect and the score column that confirmed it.
...
**Every action MUST comply with all constraints defined in the Spec Schema** (global constraints, per-component rules, cross-component invariants). Re-read the schema before finalizing each action.
...
- The current spec does NOT contain the HAVING verification rule → **diagnosis H: lost win**.
