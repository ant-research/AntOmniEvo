...
- agent proposer(抽象名;`antomnievo/proposer/` 里 `BaseProposer` 的具体实现,以已装包实际为准、不写死实现名)构造(spec_schema/candidate_store/evaluator/system/api_key/base_url/model/provider/... 以所选实现签名) → 你要的 proposer 配置(YAML `proposer.*` + spec_def)。
- `ParetoFrontierEvolutionAlgorithm`/`Budget`/`LocalCandidateStore` 构造。
- **spec_def**(步骤 3 或本步写,`<exp_root>/antomnievo/<scenario>/spec_defs/<domain>_spec_def.py`)→ 喂给 proposer。
- 看 `Optimizer.optimize()` 主循环背的依赖 → 确认你入口备全(train/val/budget/initial_spec_dir)。
...
3. 路径(repo/源码仓库/cases/schema_manifest)+ workspace_dir + `initial_spec_dir`/`editable`。超参不问用户,先用默认(batch_size/num_proposals 用 Optimizer 默认);预算跟用户讲清支持的类型(`max_iterations`/`max_rollouts`/`max_system_runs`/`max_tokens`/`max_elapsed_seconds`),**默认时间预算 8 小时**(`max_elapsed_seconds: 28800`),并告诉用户调参位置 = config YAML 的 `hyperparams` 段。
...
⚠ `initial_spec_dir` ≠ `--agent-src`(agent module)!`initial_spec_dir` 是用脚本从 agent module **只抽出可编辑部分**生成的干净 spec 目录:
```bash
# scripts/extract_initial_spec.py — 抽取可编辑 spec
python scripts/extract_initial_spec.py \
  --agent-src src/<pkg> \  # agent module 包根
  --editable <entry1> \   # 步骤 3 对齐的可编辑条目(e.g. agents),可重复
  --output <workspace>/initial_spec  # 输出
```
- **为什么**:LocalCandidateStore `create_root` 整树 `copytree`(只 ignore `__pycache__`)→ 把 `initial_spec_dir` 整份拷进每个候 → 跑代留下基线快照。如果你把 `initial_spec_dir` 设成整个 agent module,会把 config/ + knowledge_data/*.jsonl(重数据)+ main.py/app.py(平台 bootstrap)也拷进每个候选 → ① 重数据 × 候选 × rollout = 存储爆炸、② proposer 能碰非可编辑(破坏平台 bootstrap / 运行配置)。
- **spec ≠ agent module**:spec 只是你跟用户对齐的**可编辑行为面**(步骤 3 — 如本项目的 `agents/` 子包);agent module 还包含**不可编辑**的 config/、knowledge_data/、main.py/app.py/solution.py(在 generate.py 的 `--agent-src` 里用,运行时代理包)。spec 放 `initial_spec`,agent module 在 `--agent-src`,generate.py `--editable <ent>` 把候选的可编辑 `<ent>` overlay 回运行期包跑(见步骤 4 load-spec)。
- **脚本放 `scripts/extract_initial_spec.py`**(业务仓库);`--editable` 用步骤 3 跟用户对齐的列表(e.g. `agents`);若用 `agents`+另一 `--editable <another>`,脚本会拷两份,放进 `initial_spec/<ent>` 各自 pkg-rel path 位置。
- 跑完脚本把 `<workspace>/initial_spec` 写进 YAML 的 `initial_spec_dir:` 即可;**`initial_spec_dir` 不能设成 `--agent-src`/`src/<pkg>`**。
```yaml
...
agent_src: <repo>/src/<pkg>; initial_spec_dir: <同左>; editable: ["<可编辑根>"]
...
`argparse --config`(或 env `<DOMAIN>_CONFIG`/默认 `./config.yaml`)+ `--check-config`(只校验、不跑)→ `yaml.safe_load` → 校验必填 secrets → **物化 `<workspace>/run.env`**(从 `system.theta_api_key`/`iam_token` 写 `THETA_API_KEY`/`IAM_TOKEN=` 两行)→ 构造 `System(env_file=run.env)` + Evaluator + agent proposer(`BaseProposer` 具体实现以已装包为准;`spec_schema=<spec_def>, ..., api_key/ base_url/ model/... = YAML`) + `ParetoFrontierEvolutionAlgorithm` + `LocalCandidateStore(workspace_dir)` + `<Domain>Optimizer(..., initial_spec_dir=...)` → `len(train)=train_max`/`val` 顺序子集切 → `await optimizer.optimize()`。**绝不**在代码里 `api_key="..."`。
...
4. **proposer 需 `pi` coding-agent CLI**(装好)+ proposer key;全量 `optimize()` 还要 model key + MCP 权限 + sandbox 仓库 + schema manifest + 大开销(每候选真跑 agent + LLM 改 spec),**别误跑**;先 `--check-config` 再小 budget。
...
2. **小 budget `optimize()`**(可能/重):`max_iterations=1`、`train≈2`、`val≈1`、`num_proposals=1` 跑一次 —— 看根 baseline → proposer 改 spec → 子候选 跑+评 → 比较/淘汰 循环通(proposer CLI + 全前置就绪后)。
...
- spec_def: `antomnievo/model/spec_defs/adconfig_spec_def.py`
- initial spec 抽取: `scripts/extract_initial_spec.py`
