...
对应 AntOmniEvo 优化循环里的 **System** 组件:把一个候选可调产物跑起来、产出预测(trajectory + output)给 Evaluator 评分。本配方主要覆盖**"系统 = 调外部 generate.py 子进程"**这种形态(本项目的 eddy generate.py 就是这种,对标 text2sql)。其它形态(in-process agent / 容器化)照系统形态最像的一份 example 改结构。
...
- 里面建本场景包 `<scenario>/`(如 `peizhiagent/`):System → `<scenario>/system/<domain>_system.py`,Evaluator → `<scenario>/evaluator/`,Optimizer 子类 → `<scenario>/optimizer/`,数据层 → `<scenario>/dataset/`,tunable_artifact_def → `<scenario>/tunable_artifact_defs/`,入口脚本 → `<scenario>/<domain>_optimize.py`,辅助脚本 → `<scenario>/scripts/`。
...
3. `antomnievo/model/candidate_data.py` 的 `CandidateMeta` —— **`candidate_meta.artifact_dir` 是候选的可调产物目录**(你 load 回 runtime 的那个 `--artifact-dir`)。
...
- **构造参数**(scenario-specific,构造时给,全**绝对路径**):`generate_script`、`python_path`(业务项目 venv 的 python,generate.py 的 deps 在那)、`agent_src`、`tunable`(步骤 3 约定的可调条目,list)、`local_source_repo`(写型 agent 的源码仓库)、`env_file`、`timeout`(单 case 超时)、`concurrency`(generate.py 的 case 级并发)、`local_sandbox_transport`、`no_install_deps`、`extra_args`。
- **拼命令**:`[python_path, generate_script, --agent-src, --artifact-dir <candidate_meta.artifact_dir>, --tunable <ent>...(repeat), --cases <cases_path>, --output-dir <output_dir>, --concurrency, --timeout-seconds]` + 写型加 `--local-sandbox-transport --local-source-repo` + `--env-file`(可选)+ `--no-install-deps` + extra_args。
...
- **`system_description()` + `Evaluator.scoring_criteria()` 必须准确反映 generate.py / evaluate.py 的机制**(你已写/读了这俩脚本,该懂原理 —— 见 SKILL『通用约定』):
  ...
  - 这俩字符串注入 proposer / 分析 prompt —— **错或空会让它错误归因(把系统/数据问题算到可调产物头上)**。写 System/Evaluator 时一并写好,别留空 / 泛泛。
...
- ⚠ **eddy → AntOmniEvo Trajectory 解析: 用共享 parser,不要自己重写**
  ...
  - 为什么有 `pre_reasoning`:proposer(pi+LLM)分析 agent 失败时,光看 tool 调用列表你看不到 "为什么 agent 跳过了验证、为什么选了 sibling config" 等决策原因。`pre_reasoning` 从 eddy 的 `type=reasoning` 消息流里按 tool_call 前后截一小段(val ~300 字)塞进 metadata,proposer 能看到 dive agent 思维,帮助定位可调产物问题。
...
3. **`candidate_meta.artifact_dir` = `--artifact-dir`**:候选的可调产物目录;配 `--tunable <pkg-rel>` 做 "load artifact"(把候选面替换到 runtime checkout 上)。baseline 候选的 artifact_dir 可以是 agent 包根本身(tunable as-is)。
...
8. **路径全绝对**:构造参数 + `run_batch` 的 `output_dir`/`cases_path` + `candidate_meta.artifact_dir` 全绝对路径,子进程 cwd 无关。
...
3. **端到端冒烟:实跑 `run_batch` 1 条 case**(真模型):造 `CandidateMeta(artifact_dir=baseline 包根)` + batch cases(1 条,`to_case_dict()`)+ `output_dir`(`<workspace>/tests/system_smoke/`,见 SKILL『通用约定』),`await system.run_batch(...)`;断言:
