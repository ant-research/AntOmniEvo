...
| 评估**数据** | 定义「好」的训练 / 验证样本 |
| `TunableArtifactSchema` | 把系统的 tunable artifacts 映射成一个文件目录:文件树 + 每个文件的用途 |
| **初始可调产物** | 起点 |
...
**AI 干活** —— 真正动手改的是 coding agent 扮演的 `Proposer`:读失败轨迹、定位该改哪个文件、把一次结构化的变更落成一个新候选的可调产物 —— 像人改代码一样。
